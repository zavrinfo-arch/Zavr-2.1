    ...
    <AnimatePresence>
      {!isChat && (
        <motion.nav
          ...
          className="fixed bottom-0 left-0 right-0 w-full z-40 px-6 py-5 bg-white/90 dark:bg-[#111118]/85 border-t border-black/[0.06] dark:border-white/[0.06] backdrop-blur-2xl flex items-center justify-around shadow-lg dark:shadow-[0_-10px_35px_rgba(0,0,0,0.5)]"
        >
          {navItems.map((item, i) => {
            if (i === 2) {
              return (
...
export function Layout({ children, onPlusClick }: { children: React.ReactNode, onPlusClick: () => void }) {
  const location = useLocation();
  const isChat = location.pathname.includes('/zettl/chat/');
  const hasProfileHeader = !isChat;

  return (
    <div className={cn(
      "max-w-[390px] mx-auto relative bg-zinc-50 dark:bg-[#0a0a0f] text-zinc-900 dark:text-white font-sans antialiased w-full min-h-screen shadow-2xl",
      isChat 
        ? "h-screen p-0 m-0 flex flex-col overflow-hidden" 
        : (location.pathname.includes('/zettl') ? "pt-4 pb-24 px-5 overflow-x-hidden" : "pt-28 pb-24 px-5 overflow-x-hidden")
    )}>
      {hasProfileHeader && <ProfileHeader />}
      <main key={location.pathname} className={cn("page-transition w-full flex-1 flex flex-col", isChat ? "h-full w-full p-0 m-0 overflow-hidden" : "")}>
        {children}
      </main>
